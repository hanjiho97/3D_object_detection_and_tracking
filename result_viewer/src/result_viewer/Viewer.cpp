...
bool Viewer::load_resource_images(const std::string& resource_path)
{
  ...
  {
    spdlog::error("cannot load image from resource_path : {}{}", resource_path, "background.png");
    return -1;
  }
  cv::Mat car_image;
  ...
  for (uint8_t car_number=1; car_number<15; ++car_number)
  {
    ...
    if (background_.empty())
    {
      spdlog::error("cannot load image from resource_path : {}{}", resource_path, file_name);
      return -1;
    }
...
{
  cv::Point centor_point = (topview_bbox_points[2] + topview_bbox_points[3]) / 2;
...
void Viewer::draw_topview_position(const std::vector<cv::Point>& topview_bbox_points)
{
  cv::Point centor_point = (topview_bbox_points[3] + topview_bbox_points[4]) / 2;
  ...
  bool showing_topview_car)
{
  ...
  for(auto& attributes_pair : attributes_list_)
  {
    ...
    top_view_bbox_points = projection_.get_topview_conrers(
      BACKGROUND_HALF_WIDTH,
      BACKGROUND_HEIGHT,
      BACKGROUND_BOX_SCALE);
...
void Viewer::add_attributes_list(const std::map<uint, Attributes>& attributes_list)
{
  ...
  bool showing_topview_car)
{
  draw(
    ...
    showing_topview_car);
  if ((showing_topview_box == true) || (showing_topview_id == true) || 
