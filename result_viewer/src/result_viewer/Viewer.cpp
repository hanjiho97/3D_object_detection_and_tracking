...
void Viewer::draw_3d_bbox(cv::Mat& image, const std::vector<cv::Point>& bbox_points)
{
  for (uint8_t index=0; index < bbox_points.size()-4; index+=2)
  {
    cv::line(image, bbox_points[index], bbox_points[index+1], cv::Scalar(255, 0, 255), 2);
  }
  return;
}

void Viewer::draw_3d_bbox_head(cv::Mat& image, const std::vector<cv::Point>& bbox_points)
{
