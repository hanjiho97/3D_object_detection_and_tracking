    ...
    const Attributes& attributes)
{
  ...
  corners1_3D.row(0) << -length/2, -length/2, -length/2, -length/2, -length/2, -length/2, length/2, 
                         length/2, -length/2, length/2, length/2, -length/2, length/2, length/2;
  corners1_3D.row(1) << -width, -width, 0, 0, 0, -width, -width, 
                        0, -width, -width, -width, -width, -width, 0;
  corners1_3D.row(2) << -height/2, height/2, height/2, -height/2, height/2, height/2, height/2, 
                         height/2, -height/2, -height/2, height/2, height/2, height/2, height/2;
                        
  corners2_3D.row(0) << length/2, length/2, length/2, length/2, -length/2, -length/2, length/2,
                        length/2, -length/2, length/2, length/2, -length/2, length/2, length/2;
  corners2_3D.row(1) << -width, -width, 0, 0, 0, -width, -width, 
                        0, 0, 0, 0, 0, 0, -width;
  corners2_3D.row(2) << -height/2, height/2, height/2, -height/2, -height/2, -height/2, -height/2, 
                        -height/2, -height/2, -height/2, height/2, height/2, -height/2, -height/2;
  return;
}
...
{
  Eigen::Matrix<double, 4, 14> corners = Eigen::Matrix<double, 4, 14>::Constant(1);
...
std::vector<cv::Point> Projection::get_2D_corners()
{
  ...
  for (uint8_t col=0; col<14; ++col)
  {
    point.x = static_cast<uint16_t>(corners1_2D_(0, col) / corners1_2D_(2, col));
    point.y = static_cast<uint16_t>(corners1_2D_(1, col) / corners1_2D_(2, col));
    points.push_back(point);

    point.x = static_cast<uint16_t>(corners2_2D_(0, col) / corners2_2D_(2, col));
    point.y = static_cast<uint16_t>(corners2_2D_(1, col) / corners2_2D_(2, col));
    points.push_back(point);
  }
  return points;
}