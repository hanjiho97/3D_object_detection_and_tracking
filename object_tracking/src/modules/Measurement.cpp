...
Measurement::Measurement(int frame_cnt, const Kitti_Object &obj, const Kitti_Calib &calib)
{
    R_ = Eigen::MatrixXd::Zero(3, 3);
    double sigma_lidar_x = 0.1;
    double sigma_lidar_y = 0.1;
...
void Measurement::print()
{
    ...
    std::cout << "z_ = " << std::endl
              << z_ << std::endl;
    std::cout << "cam_to_veh_ = " << std::endl
              << cam_to_veh_ << std::endl;
    std::cout << "veh_to_cam_ = " << std::endl
              << veh_to_cam_ << std::endl;
    std::cout << "R_ = " << std::endl
              << R_ << std::endl;
    std::cout << "t_ = " << std::endl
              << t_ << std::endl;
    std::cout << "class_num_ = " << std::endl
              << type_ << std::endl;
...
void load_kitti_label(const std::string &label_path, std::vector<Kitti_Object> &objs)
{
    ...
    while (std::getline(ifs, line))
    {
        ...
        ss >> obj.type >> obj.truncated >> obj.occluded >> obj.alpha >>
            ...
            obj.width >> obj.length >> obj.loc_x >> obj.loc_y >>
            obj.loc_z >> obj.rot_y >> obj.score;
        objs.push_back(obj);
    }
    ifs.close();
}
...
Kitti_Calib load_kitti_calib(const std::string &calib_path)
{
    ...
    std::cout << "R0" << std::endl
              << calib.R0_rect << std::endl;
    std::cout << "velo2cam" << std::endl
              << calib.velo_to_cam << std::endl;
    std::cout << "imu2velo" << std::endl
              << calib.imu_to_velo << std::endl;
